...
class AppDatabase: AppDatabaseInterface, AppDatabaseSynchronousInterface {
    ...
    func exportDatabaseContent(from dbPool: DatabaseWriter) throws -> String {
        let exportName = ProcessInfo.processInfo.globallyUniqueString + ".sqlite"
        ...
        //try dbPool.backup(to: DatabaseQueue(path: exportPath))
        
        let destination = try DatabaseQueue(path: exportPath)
        try dbPool.write { sourceDb in
